...
static int parse_relative(const char *input, time_t *out, char *errbuf, size_t errsz)
{
    ...
    while (*p) {
        ...
        char unit = *end;
        switch (unit) {
            case 'd': if (seen_d) goto dup; seen_d = 1; secs += v * 86400; break;
            case 'h': if (seen_h) goto dup; seen_h = 1; secs += v * 3600;  break;
            case 'm': if (seen_m) goto dup; seen_m = 1; secs += v * 60;    break;
            case 's': if (seen_s) goto dup; seen_s = 1; secs += v;         break;
            default:
...
static int parse_clock(const char *input, time_t *out, char *errbuf, size_t errsz)
{
    int h, m, s = 0;
    int consumed = 0;
    if (sscanf(input, "%d:%d:%d%n", &h, &m, &s, &consumed) == 3
        && input[consumed] == '\0') {
        /* matched HH:MM:SS */
    } else {
        consumed = 0;
        s = 0;
        if (sscanf(input, "%d:%d%n", &h, &m, &consumed) != 2
            || input[consumed] != '\0') {
...
void format_duration(long seconds, char *buf, size_t n)
{
    ...
    size_t off = 0;
    if (h > 0) off += snprintf(buf + off, off < n ? n - off : 0, "%ldh ", h);
    if (m > 0) off += snprintf(buf + off, off < n ? n - off : 0, "%ldm ", m);
    if (s > 0) off += snprintf(buf + off, off < n ? n - off : 0, "%lds", s);

    if (off > 0 && buf[off - 1] == ' ') { buf[--off] = '\0'; }
    if (neg) snprintf(buf + off, off < n ? n - off : 0, " ago");
}
